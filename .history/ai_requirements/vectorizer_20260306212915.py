...
vocab = sorted(vocab_to_index)

# The rest of this file is commented. This is because the vectorizer is a file that has several variables e.g. vocab_t
...
# # # Dumping results into idfs_dict.py
# # import json
# # with open("ai_requirements/tfidf_vector/idfs_dict.py", "w", encoding="utf-8") as f:
# #     # We first write a definition so that the file is valid syntax in Python.
# #     f.write("global_idfs_dict = ")
# #     # Then we dump the actual dictionary.
# #     json.dump(global_idfs_dict, f, indent=4, ensure_ascii=False)

# # Helper function to multiply TF by IDF.
...
# # We want to construct a sparse matrix to vastly optimize computations later.
# # We save everything to sparseparts.csv then construct the final matrix from that.
# # sparseparts.csv ends up being 7767012 lines.
# data, rows, cols = [], [], []
# for doc_idx, doc in enumerate(tokenized_corpus):
#     if doc_idx % 1000 == 0 and doc_idx > 0:
#         print(f"Flushing documents from {doc_idx - 1000} to {doc_idx} to sparse_parts.csv.")
#         # We create a frame with the rows, then columns, then data.
#         buffer = pd.DataFrame({
#             "rows": rows,
#             "cols": cols,
#             "data": data,
#         })
#         # This allows me to easily transport the data into the csv.
#         buffer.to_csv("sparse_parts.csv", mode="a", header=(doc_idx == 1000), index=False)
#         # Clear rows, cols, data so that the next buffer starts from empty.
#         rows.clear(); cols.clear(); data.clear()
#
#     # Compute the TF dictionary for every document.
#     current_tf_dict = compute_tf_dict(doc)
#     # Calculate the TF-IDF dictionary entry for every document.
...
# # Finally, read in the CSV (this was done on a separate run).
# df = pd.read_csv("ai_requirements/tfidf_vector/sparse_parts.csv")
# # Create the sparse matrix and save it.
