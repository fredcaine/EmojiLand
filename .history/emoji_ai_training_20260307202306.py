...
for emoji in emojis:
    ...
    while len(valuation) < 3:
        for i in range(len(y_true) - 1, 0, -1):
            if y_true[i] == 1 and inputs[i] not in valuation:
                valuation.append(inputs[i])
                val_y_true = y_true[i]

    for epoch in range(epochs):

        inputs2 = inputs - valuation
        y_true2 = y_true - val_y_true
        logits = inputs2 @ weights + bias 
        y_pred = sigmoid(logits)
        errors = (y_pred - y_true)/(y_pred * (1 - y_pred))
        ...
        if epoch % 50000 == 0:
            ...
            print(f"Training: {emoji}. Epoch: {epoch}. Current BCE loss: {bce(y_pred, y_true)}")
