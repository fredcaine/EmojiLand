...
for emoji in emojis:
    ...
    while len(valuation) < 3:
        for i in range(len(total_y_true) - 1, 0, -1):
            if total_y_true[i] == 1 and inputs[i] not in valuation:
                valuation.append(inputs[i])
                val_y_true = total_y_true[i]
                train_y_true = np.delete(train_y_true, i, axis=0)
                train_inputs = np.delete(train_inputs, i, axis=0)

    valuation_inputs = np.array(valuation_inputs)
    valuation_labels = np.array(valuation_labels)
    ...
    for epoch in range(epochs):
        ...
        if epoch % 50000 == 0:
            ...
            print(f"Training: {emoji}. Epoch: {epoch}. Current BCE loss: {bce(y_pred, val_y_true)}")
