...
trained_data = np.load("emoji_aigen_model_weights.npz")["weights"]
...
def predict(prompt, emoji, predictions, rounding=False):
    ...
    print("input shape:",inputs.shape, "weights shape",weights.shape)
...
def emoji_dict(prompt):
    # The emoji 
    emoji_preds = create_emoji_prediction_vector(create_prediction_vector(prompt)[1:])
