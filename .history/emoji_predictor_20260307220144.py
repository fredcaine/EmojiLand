...
# This gave me a faster test than checking sentences.
emojis = ["❤","😄","😐","😠","😢","😨","😮","🤔","🤢","🤩"]
...
for emoji in emojis:
    ...
    print(f"Accuracy for {emoji}: {round(100 * np.sum(y_pred == y_true) / len(predictions), 1)}%. Average prediction: {round(np.mean(y_pred), 4)}")
