...
for emoji in emojis:
    ...
    print(f"Accuracy for {emoji}: {round(100 * np.sum(y_pred == y_true) / len(predictions), 1)}%.\t\tAverage prediction: {round(np.mean(y_pred), 4)}.\t\tPercentage frequency in dataset: {round(100 * np.sum(y_true) / len(predictions), 1)}.")
