...
import numpy as np
from emotion_predictor import create_prediction_vector, create_prediction_dicts
...
trained_data = np.load("emoji_model_weights_handwritten.npz")["arr_0"]
...
def emoji_dict(prompt):
    ...
    emoji_preds = create_emoji_prediction_vector(create_prediction_vector(prompt, rounding=False))
    emoji_prediction_dict = {e:round(100 * p, 4) for e, p in zip(emojis, emoji_preds)}
...
for emoji in emojis:
    ...
    print(f"Accuracy for {emoji}: {round(100 * np.sum(y_pred == y_true) / len(predictions), 1)}%.\t\tPercentage frequency in dataset: {round(100 * np.sum(y_true) / len(predictions), 1)}%.\t\tPercentage positive predictions: {round(100 * np.sum(y_pred) / len(predictions), 1)}%.")
    print(f"Percentage correct true predictions: {round(100 * np.sum((y_pred == y_true) & (y_pred == 1)) / sum(y_pred), 1)}%.")
...
while True:
    ...
    print("Percentage likelihoods of each emoji:")
