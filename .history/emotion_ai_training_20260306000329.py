...
emotions = ["anger", "annoyance", "approval", "caring", "confusion", "curiosity", "desire", "disappointment",
...
# weights = np.zeros(shape=(len(corpus), 1))
...
# Here is a, much more standard, method, where calculations are done through array mathematics instead of individually.
    for epoch in range(epochs):
        ...
        eps = 1e-7
        errors = (y_pred - y_true)/(y_pred * (1 - y_pred) + eps)
        ...
        if (current_epochs + epoch) % 10000 == 0:
            print(f"emotion {emotion}: epoch {current_epochs + epoch}, batch BCE: {np.mean(bce(y_pred, y_true)):.4f}")
