...
def filter_to_half(inputs, val_frac=0.2):
    ...
    # Number of validation samples (e.g., 20% of filtered data)
    val_count = int(len(filtered_rows) * 0.2)

    # Split: first val_count rows go to validation, the rest to training
...
for emotion in emotions:
    ...
    if os.path.exists(npz_file):
        ...
        print(f"Resuming {emotion} from epoch {current_epochs}")
    else:
    ...
    # This exception is mostly KeyboardInterrupt so I can pause.
        ...
        print(f"\nTraining paused at epoch {epoch}. Saving progress...")
        np.savez(npz_file, weights=weights, bias=bias, epochs=epoch)
        continue

    # Save after finishing all epochs, just in case.
