...
from ai_requirements.vectorizer import corpus, vocab

# For clarity, this file refers to 'documents' or 'docs'. These are simply Reddit messages from the GoEmotions dataset.
# Here is a link to the GoEmotions dataset. It is also found in ai_requirements/go_emotions_dataset, along with the vectorizer.
...
# This is class imbalance bias. I countered in two ways.
...
# NOTE: After a few more training loops, I decided to remove the pos_weight.
# I found that, these two adjustments actually overadjusted for overfitting, and were now a little less accurate.
# The easiest way to remove pos_weight is to set it to 1, so you will see later on as well that my weighting is 1.

def bce(y_pred, y_true, pos_weight=1):
...
for emotion in emotions:
    ...
    # The TF-IDF vector is explained in vectorizer.py
    ...
    # Apply the half + validation split defined before.
    ...
    # Otherwise, we simply start from the beginning.
    else:
        current_epochs = 0
        # The size of the weights is with the length of the vocabulary.
        # The model's goal is to tokenize then vectorize text, then multiply by the weights.
        # The size of the input is (doccount, tokencount).
        # Thus, to get a final output of a label for each doc, we multiple this by weights of size:
    ...
    # Just defining this variable so it exists. We don't need it yet.
    val_bce = float("inf")
    for epoch in range(current_epochs, epochs):
        ...
        if epoch % 1000 == 0:
            # Calculating the BCE for training.
            train_bce = np.mean(bce(y_pred, y_train))

            # Running a quick forward pass on the validation to calculate its BCE.
            val_logits = x_val @ weights
            val_pred = sigmoid(val_logits).flatten()

            # We first check if the validation loss is worse or equal to the one 1000 epochs before.
            if round(np.mean(bce(val_pred, y_val)), 4) >= round(val_bce, 4):
                # If it is, then we stop right here and save the file.
                print("Early stopping mechanism activated.")
                print(f"{emotion}.npz has been saved.")
                np.savez(npz_file, weights=weights, bias=bias, epochs=epochs)

                with open("emotion_model_training_logs.txt", "a") as f:
                    f.write(f"""The model for {emotion} was trained over {epoch} epochs.
Its training stagnated with a training BCE loss of {train_bce:.4f}, and a valuation BCE loss of {round(np.mean(bce(val_pred, y_val)), 4)}.

""")
                break
            else:
                # Otherwise, we just update the valuation BCE and print a log in case the user wants to interfere.
...
Train BCE: {train_bce:.4f}, Valuation BCE: {val_bce:.4f}""")
    ...
    np.savez(npz_file, weights=weights, bias=bias, epochs=epochs)
    print("Epochs completed. File saved.")

# At this point, all emotions have been trained to the best point before overfitting due to the early stopping mechanism.
