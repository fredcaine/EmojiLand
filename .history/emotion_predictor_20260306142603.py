...
def predict(prompt, emotion, predictions, rounding=False):
    ...
    if rounding == True:
        predictions.append(np.clip(round(prediction * 4), 0, 1))
    elif rounding:
        predictions.append(np.clip(float(round(prediction * 4, rounding)), 0, 1))
    else:
        predictions.append(np.clip(float(prediction * 4), 0, 1))
