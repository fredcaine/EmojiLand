...
def predict(prompt, emotion, predictions, predictions2, rounding=False):
    ...
    if rounding == True:
        ...
        predictions2.append(float(np.clip(round(prediction2 * 4)), 0, 1))
    elif rounding:
        predictions.append(float(np.clip(round(prediction * 4, rounding), 0, 1)))
        predictions2.append(float(np.clip(round(prediction2 * 4, rounding), 0, 1)))
    else:
        predictions.append(float(np.clip(prediction * 4), 0, 1))
        predictions2.append(float(np.clip(prediction2 * 4), 0, 1))
