...
# We first vectorize input by the same logic as ai_requirements/vectorizer.py
...
def predict(prompt, emotion, predictions, predictions2, rounding=False):
    ...
    if rounding == True:
        ...
        predictions2.append(float(np.clip(round(prediction2)), 0, 1))
    elif rounding:
        predictions.append(float(np.clip(round(prediction * 4, rounding), 0, 1)))
        predictions2.append(float(np.clip(round(prediction2, rounding), 0, 1)))
    else:
        predictions.append(float(np.clip(prediction * 4), 0, 1))
        predictions2.append(float(np.clip(prediction2), 0, 1))
...
def create_prediction_dict(prompt):
    return {e:p for e, p in zip(emotions, create_prediction_vector(prompt)[0])}, {e:p for e, p in zip(emotions, create_prediction_vector(prompt)[1])}
