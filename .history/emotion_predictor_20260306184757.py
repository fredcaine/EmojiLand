...
def predict(prompt, emotion, predictions, predictions2 = None, rounding=False):
    ...
    if rounding == True:
        ...
        predictions2.append(float(np.clip(round(prediction2)), 0, 1)) if predictions2 else pass
    elif rounding:
        predictions.append(float(np.clip(round(prediction * 4, rounding), 0, 1)))
        predictions2.append(float(np.clip(round(prediction2, rounding), 0, 1))) if predictions2 else pass
    else:
        predictions.append(float(np.clip(prediction * 4), 0, 1))
        predictions2.append(float(np.clip(prediction2), 0, 1)) if predictions2 else pass
