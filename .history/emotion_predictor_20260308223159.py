...
def predict(prompt, emotion, predictions, predictions2 = None, rounding=False):
    ...
    # Yes, 'rounding' abuses dynamic typing a little bit, but it's good.
    if rounding == True:
        predictions.append(float(np.clip(round(prediction * 4 - 2), 0, 1)))
        if predictions2 is not None: predictions2.append(float(np.clip(round(prediction2), 0, 1)))
    elif rounding:
        predictions.append(float(np.clip(round(prediction * 4 - 2, rounding), 0, 1)))
        if predictions2 is not None: predictions2.append(float(np.clip(round(prediction2, rounding), 0, 1)))
    else:
        predictions.append(float(np.clip(prediction * 4 - 2, 0, 1)))
...
def create_prediction_vector(prompt, comparison=False, rounding=False):
    if comparison:
    ...
    predictions = []
    for emotion in emotions:
        predictions = predict(prompt, emotion, predictions, rounding=rounding)
    return predictions

# Again, if I was not evaluating, I would just not return the second item.
def create_prediction_dicts(prompt, comparison=False):
    ...
    if comparison:
        return "\n\nValues for newer model:\n\n", {e:p for e, p in zip(emotions, create_prediction_vector(prompt, comparison=True, rounding=3)[0])}, "\n\nValues for older model:\n\n", {e:p for e, p in zip(emotions, create_prediction_vector(prompt)[1])}
    else:
        return {e:p for e, p in zip(emotions, create_prediction_vector(prompt, rounding=3))}
