...
def classify_emotions(emotions: Dict[str, float], explain: bool=False) -> Tuple[str, Dict[str, float]]:
    ...
    for emo_name, value in e.items():
        contrib = emotion_contributions.get(emo_name, {})
...
while True:
    prompt = input("Enter your text (type 'exit' to quit): ")
    if prompt.lower() == "exit":
        break
    emoji_dict = create_prediction_dict(prompt)
    emoji, scores = classify_emotions(emoji_dict, explain=True)
