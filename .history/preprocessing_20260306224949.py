...
# This file is quite diffiult to use, but the majority is just different input generators that aren't of much use as the inputs are given in the root folder.
...
# UNCOMMENT THIS FOR CONVERSION OF WEIGHTS CSV-->NPZ
db = pd.read_csv("model_weights.csv")
emoji_cols = [c for c in db.columns if c != "emotion"]
bias = db.loc[db["emotion"] == "bias", emoji_cols].to_numpy(dtype=np.float32)[0]
weights = db.loc[db["emotion"] != "bias", emoji_cols].to_numpy(dtype=np.float32)
np.savez("emoji_aigen_model_weights.npz", weights=weights, bias=bias)
