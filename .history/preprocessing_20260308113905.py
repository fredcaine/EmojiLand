...
# This file is quite difficult to use, but the majority is just various input generators that aren't of much use as the inputs are given in the folder.
# It is kept solely for my reference.
...
# UNCOMMENT THIS TO CONVERT MODEL_WEIGHTS.CSV TO .NPZ
db = pd.read_csv("emoji_model_weights_handwritten.csv")
finish = db.drop(columns="emotion").to_numpy(dtype=float)
np.savez("emoji_model_weights_handwritten.npz", finish)
print("Preprocessing completed.")

# UNCOMMENT THIS FOR A DB INPUT
# finish = db.drop(columns="text").drop(columns="id").drop(columns="neutral").astype(int).iloc[:294].values
