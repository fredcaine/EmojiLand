...
This then serves as a weight on each token that signifies its importance.  

At this point, we train a model that calculates model weights. These give a value to each token so that the weights multiply by the TF-IDF vector to give a single prediction for each document.  
...
This is especially so because of the English language. After all, important phrases are often three words long e.g.  
...
There were pros for both.  

For dataset inputs, the inputs were guaranteed to be accurate for each emotion, so the AI could really learn the patterns of converting emotion to emoji.  
